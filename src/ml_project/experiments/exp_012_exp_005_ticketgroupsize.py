...
EXPERIMENT = ExperimentSettings(
    ...
    experiment_title='EXP-005+ticketGroupSize',
    experiment_note=Path(
        "experiments/EXP-012 Exp 005 Ticketgroupsize.md"
    ),
    ...
    change_description="Создать переменную TicketGroup, которая будет группировать билеты по номеру билета,и рассчитать FarePerPerson и преобразовать log1p относительно EXP003 больше ничего не менять" ,
    success_criterion=(
...
def prepare_candidate_data(
    ...
    frame["TicketGroupSize"] = (
    frame.groupby("Ticket")["Ticket"]
    .transform("size")
)
    frame["FarePerPerson"]=np.log1p(frame["Fare"]/frame["TicketGroupSize"])
    groups["numeric"] = [
            *groups.get("numeric", []),
            "FarePerPerson",
        ]
    groups["count"] = [*groups.get("count", []), "TicketGroupSize"]
